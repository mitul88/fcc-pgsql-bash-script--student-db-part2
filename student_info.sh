...
echo "$($PSQL "SELECT course FROM courses FULL JOIN majors_courses using(course_id) FULL JOIN students using(major_id) WHERE (first_name='Obie' AND last_name='Hilpert') OR student_id IS NULL GROUP BY course ORDER BY course DESC")"
